...
func before() {
	...
	bytesEntrada, err := ioutil.ReadFile("./server/database/database.txt")
	error := false
	if err != nil {
		error = true
	}
	if error == true || len(string(bytesEntrada)) == 0 {
		//fileData := []byte("{}")
		ioutil.WriteFile("./server/database/database.txt", []byte(""), 0644)
	} else {
...
// CreateCardVaultEntry crea una entrada de tipo tarjeta de credito
func CreateCardVaultEntry(email string, entryTitle string, cardDigits string, cardExpiration string, cardCCV string) error {
	var errResult error
...
func After() {
	salida, err := os.Create("./server/database/database.txt")
	if err != nil {
		panic(0)
	}
